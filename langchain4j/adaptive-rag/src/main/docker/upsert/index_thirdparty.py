...
def upsert_chroma_via_http( client, docs, embedding_function ):
    ...
    collection = client.create_collection("rag-chroma", embedding_function=CustomOpenAIEmbeddings(model=embedding_model, base_url=base_url,api_key=api_key))
    for doc in docs:
        collection.add(
            ids=[str(uuid.uuid1())],
...
embd = CustomOpenAIEmbeddings(model=embedding_model, base_url=base_url,api_key=api_key)
